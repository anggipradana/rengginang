function get_external_tool_latest_version(tool_id, tool_name) {
  ...
  if (
    ...
    Swal.fire({
      ...
      text: `Since the version lookup command is invalid, ReNgGinaNg is not able to detect if there's a newer version. But you can still force download the latest version.`,
      icon: "info",
    ...
    fetch("/api/github/tool/get_latest_releases/?tool_id=" + tool_id)
      ...
      .then(function (response) {
        ...
        if (response["message"] == "RateLimited") {
          ...
          latest_version =
            latest_version.charAt(0) == "v" ? latest_version.substring(1) : latest_version;
          ...
          if (
            ...
            Swal.fire({
              title: "Unable to fetch latest version!",
              text: `Since the version lookup command is invalid, ReNgGinaNg is not able to detect if there's a newer version. But you can still force download the latest version. The latest version is ${latest_version}.`,
              icon: "info",
              confirmButtonText: "Update " + htmlEncode(tool_name),
            }).then((result) => {
              /* Read more about isConfirmed, isDenied below */
            ...
            current_version =
              current_version.charAt(0) == "v" ? current_version.substring(1) : current_version;
...
function get_external_tool_current_version(tool_id, id) {
  fetch("/api/external/tool/get_current_release/?tool_id=" + tool_id)
    ...
    .then(function (response) {
      if (response["status"]) {
        version_number =
          response["version_number"].charAt(0) == "v" || response["version_number"].charAt(0) == "V"
            ? response["version_number"]
            : "v" + response["version_number"];
        document.getElementById(id).innerHTML =
          '<span class="badge badge-soft-primary">' + version_number + "</span>";
      } else {
        document.getElementById(id).innerHTML =
          '<span class="badge badge-soft-danger">' + response["message"] + "</span>";
      }
    });
}
