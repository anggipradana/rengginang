          ...
          if (row["http_url"]) {
            if (row["cname"]) {
              return (
                ...
                row["cname"].replace(",", '<br><span class="text-warning"> ❯ </span>') +
                `</span>`
              );
            }
            return (
  ...
  $.getJSON(`/api/listSubdomains/?scan_id=${scan_id}&no_page&only_screenshot`, function (data) {
    ...
    if (service_select) {
      for (var service in service_array) {
        if (
          !$("#services_select_filter").find("option:contains('" + service_array[service] + "')")
            .length
        ) {
          var option = document.createElement("option");
  ...
  $.getJSON(`/api/queryMetadata/?scan_id=${scan_id}&format=json`, function (data) {
    ...
    for (var val in data["metadata"]) {
      ...
      if (doc["producer"] || doc["creator"] || doc["os"]) {
        ...
        metadata += doc["os"]
          ? `<br> <span class='badge badge-soft-danger'> OS: ` + doc["os"] + "</span>"
          : "";
  ...
  $.getJSON(`/api/queryDorks/?scan_id=${scan_id}&format=json`, function (data) {
    ...
    for (var val in data["dorks"]) {
      ...
      $("#dork_type_vertical_tablist").append(
        `<a class="nav-link ${active} mb-1" id="v-${val}-tab" data-bs-toggle="pill" href="#v-${val}" role="tab" aria-controls="v-${val}" aria-selected="true"> ${convertToCamelCase(val)}</a>`
      );
      // create tab content
...
function detect_subdomain_cms(http_url, http_status) {
  if (http_status == 0) {
    var message = `ReNgGinaNg has earlier identified that this subdomain did not return any HTTP status and likely the subdomain is not alive. ReNgGinaNg may not be able to detect any CMS, would you still like to continue?`;
  } else if (http_status != 200) {
    var message = `ReNgGinaNg has earlier identified that this subdomain has HTTP status as ${http_status} and likely that ReNgGinaNg will not detect any CMS, would you still like to continue?`;
  }
