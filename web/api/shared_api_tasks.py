...
from ReNgGinaNg.common_func import create_inappnotification, get_hackerone_key_username
from ReNgGinaNg.definitions import PROJECT_LEVEL_NOTIFICATION, HACKERONE_ALLOWED_ASSET_TYPES
from ReNgGinaNg.celery import app
from ReNgGinaNg.database_utils import bulk_import_targets
...
def import_hackerone_programs_task(handles, project_slug, is_sync = False):
	...
	for handle in handles:
		...
		if program_details:
			...
			# via PR https://github.com/anggipradana/ReNgGinaNg/pull/1410
			try:
				...
				for scope in scopes:
					...
					# we need to filter the scope that are supported by ReNgGinaNg now
