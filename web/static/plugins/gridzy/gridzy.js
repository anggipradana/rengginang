...
(function () {
  ...
  var helper = {
    ...
    changeMultipleClassNames: (function () {
      // IE11 support for adding multiple css classes
                  ...
                  if (type === undefined || type === eventListener[3]) {
                    if (handler === undefined || handler === eventListener[4]) {
                      if (options === undefined || options === eventListener[5]) {
                        eventListener[2].removeEventListener(
                          eventListener[3],
                          eventListener[4],
                          eventListener[5]
                        );
    ...
    completeDefaultOptionsObject: function () {
      if (
        ...
        typeof Gridzy._layouts[Gridzy._defaultOptions.layout] !== "undefined"
      ) {
        Gridzy._defaultOptions = helper.optionsMerge(
          helper.cloneDefaultOptionsTemplate(Gridzy._defaultOptions.layout),
          Gridzy._defaultOptions
        );
        defaultOptionsCompleted = Gridzy._defaultOptions.layout;
      }
    },
    cloneDefaultOptionsTemplate: function (layout) {
      if (
        typeof defaultOptionsLayoutTpl[layout] === "undefined" &&
        typeof Gridzy._layouts[layout] !== "undefined"
      ) {
        defaultOptionsLayoutTpl[layout] = helper.objectAssign(
          {},
          Gridzy._layouts[layout]._defaultOptions,
          defaultOptionsTpl
        );
      }
      return typeof defaultOptionsLayoutTpl[layout] !== "undefined"
        ? helper.objectAssign({}, defaultOptionsLayoutTpl[layout])
        : {};
    },
  };
  ...
  function GridzyRenderObject(gridzyInstance) {
    ...
    dimensions.actualBox =
      style.boxSizing === "border-box" ? dimensions.borderBox : dimensions.contentBox;
  ...
  window.GridzyLayout = function (parameter) {
    if (
      ...
      typeof Gridzy._layouts[parameter.name] === "undefined"
    ) {
      this.name = parameter.name;
      this._defaultOptions =
        parameter && typeof parameter.options === "object" ? parameter.options : {};
      this._handler =
        parameter && typeof parameter.handler === "function" ? parameter.handler : function () {};
  ...
  window.Gridzy = function (element, options) {
    ...
    if (isNewInstance) {
              ...
              switch (mutation.type) {
                case "attributes":
                  if (mutation.attributeName === "class" && self._options.useOptionAttributes) {
                    if (
                      self._options.animate !==
                        mutation.target.classList.contains("gridzyAnimated") &&
                    ...
                    mutation.attributeName.indexOf("data-gridzy-") === 0 &&
                    self._options.useOptionAttributes
                  ) {
                    var optionData = self._getOptionParametersFromElement(
                      null,
                      null,
      ...
      for (visibilityClassName in self._itemVisibilityClasses) {
        self._itemVisibilityClasses[visibilityClassName].remove =
          self._itemVisibilityClassesArray.filter(function (el) {
      ...
      helper.eventListener.add(self, null, self.element, "animationend", function (event) {
        if (event.target.gridzyItem) {
          ...
          helper.changeMultipleClassNames(
            event.target,
            true,
            self._itemVisibilityClasses[JSON.stringify(event.target.gridzyItem.visible)].add
          );
          helper.changeMultipleClassNames(
            event.target,
            false,
            self._itemVisibilityClasses[JSON.stringify(event.target.gridzyItem.visible)].remove
          );
        }
      });
  ...
  Gridzy.setDefaultOptions = function (options) {
    if (
      ...
      typeof Gridzy._layouts[options.layout] !== "undefined"
    ) {
      Gridzy._defaultOptions.layout = options.layout;
    }
  ...
  Gridzy.getDefaultOptions = function (layout) {
    if (
      layout &&
      layout !== Gridzy._defaultOptions.layout &&
      typeof Gridzy._layouts[layout] !== "undefiend"
    ) {
      return helper.optionsMerge(
        helper.cloneDefaultOptionsTemplate(layout),
        Gridzy._defaultOptions
      );
    } else {
      helper.completeDefaultOptionsObject();
  ...
  Gridzy.getDefaultOption = function (name) {
    helper.completeDefaultOptionsObject();
    return typeof Gridzy._defaultOptions[name] !== "undefined"
      ? Gridzy._defaultOptions[name]
      : null;
  };
  ...
  Gridzy.prototype = {
    ...
    render: function () {
      ...
      while (pos--) {
        ...
        if (item.valid && !item.element.gridzyItem.visible) {
          item.element.style.cssText =
            item.element.style.cssText + "; " + item.styleToApply + "; transition: none; ";
        }
      }
      ...
      renderObject.gridzyInstance.element.style.cssText =
        renderObject.gridzyInstance.element.style.cssText + "; " + renderObject.styleToApply;
      ...
      while (pos--) {
        ...
        if (item.valid) {
          ...
          changeClasses =
            this._itemVisibilityClasses[
              JSON.stringify(item.element.gridzyItem.visible) + "_" + JSON.stringify(item.visible)
            ];
          if (changeClasses) {
    ...
    _getRenderObject: function () {
      ...
      if (autoConsiderScrollBars) {
        ...
        while (ancestor.parentNode && ancestor.parentNode.nodeName !== "#document") {
          ...
          if (
            ancestorOverflowY === "auto" ||
            (ancestor.nodeName === "BODY" && ancestorOverflowY === "visible")
          ) {
            testRequired = true;

            ancestorHeight =
              ancestor.nodeName === "BODY"
    ...
    _resetItemsDOM: function (items) {
      ...
      while (pos--) {
        ...
        helper.changeMultipleClassNames(
          item.contentElement,
          false,
          this._itemVisibilityClassesArray.concat([
            "gridzyItem",
            "gridzyItemLoading",
    ...
    _createItem: function (contentElement) {
      ...
      helper.changeMultipleClassNames(item.contentElement, true, [
        "gridzyItem",
        "gridzyItemLoading",
        "gridzyItemInitializing",
      ]);
      ...
      while (pos--) {
        (function (e) {
          ...
          if (
            ...
            parseInt(attributeWidth) > 0 &&
            parseInt(attributeHeight) > 0
          ) {
      ...
      item.images.forEach(
        function (image, index) {
          ...
          if (!item.imagesInfo[index].lazySrc && !item.imagesInfo[index].lazySrcset) {
            helper.expectNaturalImageSize(
              this,
              item,
              image,
              function (img, success) {
                item.imagesStatus[index] =
                  success && image.naturalWidth
                    ? image.complete
                      ? this.IMG_FINISHED
                      : this.IMG_READY
                    : this.IMG_FAILURE;
    ...
    _processInsertQueue: function (maxQuietTimeReached) {
      var queueItem;
      var maxQueueTimeReached =
        this._insertQueue.lastProcess < new Date().getTime() - this._insertQueue.maxQueueTime;
      ...
      if (maxQuietTimeReached || maxQueueTimeReached) {
        ...
        while (queueItemsPos--) {
          ...
          while (imagePos--) {
            ...
            if (
              !imagesInfo.attributeWidth &&
              queueItem.item.images[imagePos].offsetWidth < 40 &&
              queueItem.item.images[imagePos].naturalWidth
            ) {
              imagesInfo.attributeWidth = queueItem.item.images[imagePos].naturalWidth;
            }
            if (
              !imagesInfo.attributeHeight &&
              queueItem.item.images[imagePos].offsetHeight < 40 &&
              queueItem.item.images[imagePos].naturalHeight
            ) {
            ...
            if (imagesInfo.attributeWidth || imagesInfo.attributeHeight) {
              ...
              if (
                (parseInt(imageStyle.width) < 40 ||
                  imageStyle.width === "auto" ||
                  imagesInfo.lazySrc ||
                  imagesInfo.lazySrcset) &&
                imagesInfo.attributeWidth &&
                !imagesInfo.styleWidth
              ) {
              ...
              if (
                (parseInt(imageStyle.height) < 40 ||
                  imageStyle.height === "auto" ||
                  imagesInfo.lazySrc ||
                  imagesInfo.lazySrcset) &&
                imagesInfo.attributeHeight &&
                !imagesInfo.styleHeight
              ) {
                imagesStylesToReset.push({
                  image: queueItem.item.images[imagePos],
                  definition: "height",
        ...
        while (imagesStylesToResetPos--) {
          imagesStylesToReset[imagesStylesToResetPos].image.style[
            imagesStylesToReset[imagesStylesToResetPos].definition
          ] = imagesStylesToReset[imagesStylesToResetPos].value;
        ...
        while (queueItemsPos--) {
          queueItem = queueItems[queueItemsPos];
          queueItem.item.size = [
            queueItem.contentElement.offsetWidth,
            queueItem.contentElement.offsetHeight,
          ];
        ...
        this._insertQueue.timer = setTimeout(
          function () {
            this._processInsertQueue(true);
          }.bind(this),
          this._insertQueue.maxQuietTime
        );
      }
    },
    ...
    _onImageFinished: function (item) {
      ...
      if (
        !helper.inArray(item.imagesStatus, this.IMG_LOADING) &&
        !helper.inArray(item.imagesStatus, this.IMG_LOADING_READY) &&
        !helper.inArray(item.imagesStatus, this.IMG_READY)
      ) {
        // temporarily disable observations
        pauseMutationObservation();
    ...
    _onImageReady: function (item) {
      ...
      if (
        item.status === this.ITEM_REGISTERED &&
        !helper.inArray(item.imagesStatus, this.IMG_LOADING)
      ) {
    ...
    _getOptionParametersFromElement: function (element, layout, attributeName) {
      ...
      for (optionName in defaultOptions) {
        defaultOptionsNormalized[optionName.toLowerCase()] = {
          name: optionName,
    ...
    _updateOptionParameters: function (options) {
      ...
      if (
        ...
        typeof Gridzy._layouts[options.layout] === "object" &&
        Gridzy._layouts[options.layout].constructor === GridzyLayout &&
      ...
      if (layoutChanged) {
        ...
        if (this._options.useOptionAttributes) {
          optionParametersFromElement = this._getOptionParametersFromElement(
            null,
            this._options.layout
          );
          for (name in optionParametersFromElement) {
            if (
              typeof defaultOptions[name] !== "undefined" &&
              typeof options[name] === "undefined" &&
              this._options[name] !== optionParametersFromElement[name]
            ) {
      ...
      if (
        ...
        helper.isValidCssSelector(options.filter)
      )
        this._options.filter = options.filter;

      // set filterControls
      if (
        typeof options.filterControls === "string" &&
        (options.filterControls === "" || helper.isValidCssSelector(options.filterControls))
      )
        this._options.filterControls = options.filterControls;

      // set animate
      if (typeof options.animate !== "undefined" && options.animate !== null)
        this._options.animate = helper.isTruly(options.animate);

      // set autoConsiderScrollBars
      if (
        typeof options.autoConsiderScrollBars !== "undefined" &&
        options.autoConsiderScrollBars !== null
      )
        this._options.autoConsiderScrollBars = helper.isTruly(options.autoConsiderScrollBars);

      // set event handlers
      if (typeof options.onBeforeOptionsChanged === "function")
        this._options.onBeforeOptionsChanged = options.onBeforeOptionsChanged;
      if (typeof options.onOptionsChanged === "function")
        this._options.onOptionsChanged = options.onOptionsChanged;
      if (typeof options.onBeforeRender === "function")
        this._options.onBeforeRender = options.onBeforeRender;
      if (typeof options.onRender === "function") this._options.onRender = options.onRender;

      // set autoSyncChildListMutation
      if (
        typeof options.autoSyncChildListMutation !== "undefined" &&
        options.autoSyncChildListMutation !== null
      )
        this._options.autoSyncChildListMutation = helper.isTruly(options.autoSyncChildListMutation);

      // set autoSyncAttributesMutation
      if (
        typeof options.autoSyncAttributesMutation !== "undefined" &&
        options.autoSyncAttributesMutation !== null
      )
        this._options.autoSyncAttributesMutation = helper.isTruly(
          options.autoSyncAttributesMutation
        );

      // set autoSyncChildClassMutation
      if (
        typeof options.autoSyncChildClassMutation !== "undefined" &&
        options.autoSyncChildClassMutation !== null
      )
        this._options.autoSyncChildClassMutation = helper.isTruly(
          options.autoSyncChildClassMutation
        );
      ...
      if (
        oldOptions.filterControls !== this._options.filterControls ||
        (this._options.filterControls && this._filterControls.length === 0)
      ) {
        filterControls = [].slice.call(
          this._options.filterControls
            ? document.querySelectorAll(this._options.filterControls)
            : []
        );
            ...
            switch (this._filterControls[pos].nodeName + this._filterControls[pos].type) {
              ...
              case "INPUTradio":
                helper.eventListener.remove(
                  this,
                  null,
                  this._filterControls[pos],
                  "change",
                  this._filterControls[pos].gridzyFilterControlHandler
                );
                break;
              case "INPUTbutton":
              case "INPUTsubmit":
        ...
        while (pos--) {
          if (!helper.inArray(this._filterControls, filterControls[pos])) {
            filterControls[pos].gridzyFilterControlHandler =
              this._filterControlEventHandler.bind(this);
      ...
      if (this._options.useOptionAttributes) {
        for (name in this._options) {
          ...
          if (
            typeof options[name] !== "undefined" &&
            options[name] === null &&
            this.element.hasAttribute(attributeName)
          ) {
            this.element.removeAttribute(attributeName);
          } else if (
            defaultOptions[name] !== this._options[name] ||
            this.element.hasAttribute(attributeName)
          ) {
            switch (typeof value) {
              case "boolean":
...
new GridzyLayout({
  ...
  handler: function (renderObject) {
    ...
    singleRowAlign =
      fillSingleRow === false && singleRowAlign === "justified" ? "left" : singleRowAlign;
    ...
    optWidthRow =
      lastRowAlign === "justified"
        ? optWidthAll /
          (Math.round(
            (optWidthAll + spaceBetween * itemsLength) / (containerWidth + spaceBetween)
          ) || 1)
        : containerWidth;
    ...
    for (loopPos = 0; loopPos < rowsLength; loopPos++) {
      ...
      fillRow =
        (rowsLength === 1 && singleRowAlign === "justified") ||
        (rowsLength > 1 && lastRowAlign === "justified") ||
        loopPos < rowsLength - 1 ||
        row.optWidth >= containerWidthWithoutSpaces;

      if (
        !fillRow &&
        ((rowsLength === 1 && singleRowAlign !== "left") ||
          (rowsLength > 1 && lastRowAlign !== "left"))
      ) {
        displaySizeAllX += Math.round(
          (containerWidth - (row.optWidth + (rowLength - 1) * spaceBetween)) *
            ((rowsLength > 1 ? lastRowAlign : singleRowAlign) === "right" ? 1 : 0.5)
        );
      }
      ...
      for (loopPos2 = 0; loopPos2 < rowLength; loopPos2++) {
        ...
        item.displaySizeX = fillRow
          ? Math.round((item.aspectRatio / row.ratioAll) * containerWidthWithoutSpaces)
          : Math.round(item.optWidth);
        displaySizeAllX += item.displaySizeX;

        if (
          fillRow &&
          loopPos2 === rowLength - 1 &&
          displaySizeAllX !== containerWidthWithoutSpaces
        ) {
      ...
      for (loopPos2 = 0; loopPos2 < rowLength; loopPos2++) {
        ...
        item.styleToApply =
          ...
          "px; " +
          "left: " +
          (renderObject.dimensions.padding.left + item.displayPosX) +
          "px; " +
          "top: " +
          (renderObject.dimensions.padding.top + item.displayPosY) +
          "px; " +
          "font-size: " +
          (autoFontSize ? (item.displaySizeX * 100) / item.width + "%" : "100%") +
          "; ";
      }
    ...
    renderObject.styleToApply =
      "height: " +
      (renderObject.dimensions.actualBox.height -
        renderObject.dimensions.contentBox.height +
        renderObject.height) +
      "px; ";
...
new GridzyLayout({
  ...
  handler: function (renderObject) {
    ...
    while (loopPos--) {
      columnWidth =
        availableContainerWidth -
        Math.round((availableContainerWidth * loopPos) / columnCount) -
        usedContainerWidth;
    ...
    while (loopPos--) {
      ...
      if (horizontalOrder) {
        currentColumn =
          grid[
            currentColumn && currentColumn.index > 0 ? currentColumn.index - 1 : columnCount - 1
          ];
      } else {
        loopPos2 = columnCount - 1;
        currentColumn = grid[loopPos2];
        while (loopPos2--) {
          if (grid[loopPos2].height < currentColumn.height - 2) {
            // minus 2 to prevent to many jumps if there are many images with same size on top
            currentColumn = grid[loopPos2];
          }
        }
      }
