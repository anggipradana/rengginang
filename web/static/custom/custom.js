  ...
  var interesting_subdomain_table = $("#interesting_subdomains").DataTable({
    drawCallback: function (settings, start, end, max, total, pre) {
      ...
      if (this.fnSettings().fnRecordsTotal() == 0) {
        ...
        // <div class="card-body">
        // ReNgGinaNg could not identify any interesting subdomains. You can customize interesting subdomain keywords <a href="/scanEngine/interesting/lookup/">from here</a> and this section would be automatically updated.
        // </div>
        // </div>`);
      } else {
        // show nav bar
...
function show_subscan_results(subscan_id) {
  ...
  fetch(api_url, {
    ...
    .then(function (response) {
      ...
      $("#xl-modal-content").append(
        `<div class="mt-1">Engine Used: <span class="badge bg-primary">${htmlEncode(response["subscan"]["engine"])}</span></div>`
      );
...
function render_endpoint_in_xlmodal(endpoint_count, subdomain_name, result) {
  ...
  for (var endpoint_obj in result) {
    ...
    if (endpoint["technologies"]) {
      tech_badge =
        "<div>" + parse_technology(endpoint["technologies"], "primary", (outline = true));
    }
...
function render_vulnerability_in_xl_modal(vuln_count, subdomain_name, result) {
  ...
  for (var vuln in result) {
    ...
    var vuln_type = vuln_obj["type"]
      ? `<span class="badge badge-soft-primary">&nbsp;&nbsp;${vuln_obj["type"].toUpperCase()}&nbsp;&nbsp;</span>`
      : "";
    var tags = "";
    ...
    var vuln_title =
      `<b class="text-${color}">` + vuln_obj["name"] + `</b>` + cvss_metrics_badge + tags;
    var badge = "danger";
    var cvss_score = "";
  ...
  fetch(url, {})
    ...
    .then(function (response) {
      ...
      document.getElementById("domain_age").innerHTML =
        response["domain"]["domain_age"] + " " + response["domain"]["date_created"];
      ...
      document.getElementById("registrant_organization").innerHTML = response["registrant"][
        "organization"
      ]
    ...
    .then(function (response) {
      ...
      if (response.status) {
        ...
        $("#most_vulnerable_target_div").append(`
				...
				Once the vulnerability scan is performed, ReNgGinaNg will identify the most vulnerable targets.</div>
			`);
      }
    });
}
    ...
    .then(function (response) {
      ...
      if (response.status) {
        ...
        $("#most_common_vuln_div").append(`
				...
				Once the vulnerability scan is performed, ReNgGinaNg will identify the Most Common Vulnerabilities.</div>
			`);
      }
    });
}
  ...
  Swal.fire({
    ...
    preConfirm: (name) => {
      return fetch(`/api/action/create/project?name=${name}`)
        ...
        .catch((error) => {
          Swal.showValidationMessage(`Duplicate project name, choose another project name!`);
        });
    },
