...
function list_subdomain_todos(subdomain_id, subdomain_name) {
  ...
  $("#modal-content").append(
    `<div class='outer-div' id="modal-loader"><span class="inner-div spinner-border text-info align-self-center loader-sm"></span></div>`
  );
  // query subdomains
...
function get_task_details(todo_id) {
  ...
  $.getJSON(`/api/listTodoNotes/?todo_id=${todo_id}&format=json`, function (data) {
    ...
    if (note["subdomain_name"]) {
      subdomain_name =
        '<small class="text-success"> Subdomain: ' + note["subdomain_name"] + "</small></br>";
    }
...
function get_recon_notes(target_id, scan_id) {
  ...
  $.getJSON(url, function (data) {
    ...
    if (data["notes"].length > 0) {
      $("#todo-list").append(
        `<li class="list-group-item border-0 ps-0" id="todo_list_${target_id}"></li>`
      );
      ...
      $("#tasks-count").html(
        `<span class="badge badge-soft-primary">${data["notes"].length}</span>`
      );
    } else {
      $("#tasks-count").html(`<span class="badge badge-soft-primary me-1">0</span>`);
      $("#todo-list").append(
        `<p>No todos or notes...</br>You can add todo for individual subdomains or you can also add using + symbol above.</p>`
      );
    }
    $(".bs-tooltip").tooltip();
