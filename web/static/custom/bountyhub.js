...
document.addEventListener("DOMContentLoaded", function () {
  ...
  async function fetchPrograms(isSortingRequest = false, isBookmarkedRequest = false) {
    ...
    let api_url = isBookmarkedRequest
  ...
  function initializeFilter() {
    const filterAndSearchCards = debounce(() => {
      ...
      const visibleCards = Array.from(container.querySelectorAll(".program-card-wrapper")).filter(
        (cardWrapper) => {
      ...
      container
        .querySelectorAll(".program-card-wrapper")
  ...
  function handleCardClick(event) {
    ...
    if (card) {
      if (
        event.target.closest(".btn-see-details") ||
        event.target.closest(".handle-link") ||
        event.target.closest(".btn-import-hackerone")
      ) {
  ...
  function updateImportButton() {
    ...
    importBtn.innerHTML =
      count === 0
        ...
        : `<i class="fe-download-cloud"></i> Import ${count} Program${count !== 1 ? "s" : ""}`;
...
function populateBadges(attributes) {
  ...
  const badges = [
    ...
    {
      condition: true,
      classes: attributes.offers_bounties
        ? "bg-info text-white"
...
async function importPrograms(handles) {
  ...
  try {
    const response = await fetch(
      `/api/hackerone-programs/import_programs/?project_slug=${currentProjectSlug}`,
      {
        method: "POST",
        headers: {
