function getScanStatusSidebar(project, reload) {
  $.getJSON("/api/scan_status/?project=" + project, function (data) {
    ...
    if (scans["pending"].length > 0) {
      ...
      $("#upcoming_scans").html(
        `<div class="alert alert-info" role="alert">No upcoming Scans.</div>`
      );
    }
    if (scans["scanning"].length > 0) {
      ...
      $("#currently_scanning").html(
        `<div class="alert alert-info" role="alert">No Scans are currently running.</div>`
      );
    }
    ...
    if (scans["completed"].length > 0) {
      ...
      $("#completed").html(
        `<div class="alert alert-info" role="alert">No scans have been recently completed.</div>`
      );
    }
    ...
    if (tasks["running"].length > 0) {
      ...
      $("#currently_running_tasks").html(
        `<div class="alert alert-info" role="alert">No tasks are currently running.</div>`
      );
    }
    ...
    if (tasks["completed"].length > 0) {
      ...
      $("#completed_tasks").html(
        `<div class="alert alert-info" role="alert">No tasks have been recently completed.</div>`
      );
    }
    ...
    if (tasks["pending"].length > 0) {
      for (var task in tasks["pending"]) {
        ...
        $("#upcoming_tasks").append(
          `<div class="alert alert-warning" role="alert">${task_name} on ${task_object.subdomain_name}</div>`
        );
      }
    } else {
      $("#upcoming_tasks").html(
        `<div class="alert alert-info" role="alert">No upcoming tasks.</div>`
      );
    }
  }).done(function () {
    tippy(".scan_status", {
