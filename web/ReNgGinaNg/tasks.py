...
from ReNgGinaNg.celery_custom_task import ReNgGinaNgTask
...
def initiate_subscan(
	...
	if not method:
		logger.warning(f'Task {scan_type} is not supported by ReNgGinaNg. Skipping')
		return
...
#------------------------- #
# Tracked ReNgGinaNg tasks    #
#--------------------------#

@app.task(name='subdomain_discovery', queue='main_scan_queue', base=ReNgGinaNgTask, bind=True)
def subdomain_discovery(
	...
	for tool in tools:
		...
		if tool in default_subdomain_tools:
			...
			logger.warning(
				f'Subdomain discovery tool "{tool}" is not supported by ReNgGinaNg. Skipping.')
			continue
...
@app.task(name='osint', queue='main_scan_queue', base=ReNgGinaNgTask, bind=True)
...
@app.task(name='screenshot', queue='main_scan_queue', base=ReNgGinaNgTask, bind=True)
...
@app.task(name='port_scan', queue='main_scan_queue', base=ReNgGinaNgTask, bind=True)
...
@app.task(name='nmap', queue='main_scan_queue', base=ReNgGinaNgTask, bind=True)
def nmap(
		self,
		cmd=None,
		ports=[],
...
@app.task(name='waf_detection', queue='main_scan_queue', base=ReNgGinaNgTask, bind=True)
...
@app.task(name='dir_file_fuzz', queue='main_scan_queue', base=ReNgGinaNgTask, bind=True)
...
@app.task(name='fetch_url', queue='main_scan_queue', base=ReNgGinaNgTask, bind=True)
...
@app.task(name='vulnerability_scan', queue='main_scan_queue', bind=True, base=ReNgGinaNgTask)
...
@app.task(name='nuclei_individual_severity_module', queue='main_scan_queue', base=ReNgGinaNgTask, bind=True)
...
@app.task(name='nuclei_scan', queue='main_scan_queue', base=ReNgGinaNgTask, bind=True)
def nuclei_scan(self, urls=[], ctx={}, description=None):
	...
	only_root_urls = config.get(ONLY_ROOT_URLS, False)
	...
	if urls:
		...
		get_http_urls(
			...
			ignore_files=True,
			write_filepath=input_path,
			get_only_default_urls=only_root_urls,
			ctx=ctx
		)

	# Ensure the target domain base URL is always included in nuclei input
	# so that info-level templates (WAF, tech detect, TLS, DNS, etc.) can match
	if self.domain:
		domain_url = f'https://{self.domain.name}'
		with open(input_path, 'r') as f:
			existing_urls = f.read()
		if domain_url not in existing_urls:
...
@app.task(name='dalfox_xss_scan', queue='main_scan_queue', base=ReNgGinaNgTask, bind=True)
...
@app.task(name='crlfuzz_scan', queue='main_scan_queue', base=ReNgGinaNgTask, bind=True)
...
@app.task(name='s3scanner', queue='main_scan_queue', base=ReNgGinaNgTask, bind=True)
...
@app.task(name='wpscan_scan', queue='main_scan_queue', base=ReNgGinaNgTask, bind=True)
def wpscan_scan(self, urls=[], ctx={}, description=None):
	"""WordPress vulnerability scanning with WPScan.

	Auto-detects WordPress targets via technology fingerprints or URL patterns,
	then runs WPScan against each detected WordPress subdomain.

	Args:
		urls (list, optional): If passed, filter on those URLs.
		description (str, optional): Task description shown in UI.
	"""
	logger.info('Running WPScan WordPress Vulnerability Scan')

	# Layer 1: Detect WordPress via technology fingerprint (httpx)
	wp_subdomain_ids = set(
		Subdomain.objects.filter(
			scan_history=self.scan,
			technologies__name__icontains='wordpress'
		).values_list('id', flat=True).distinct()
	)

	# Layer 2: Detect via endpoint URL patterns (crawled URLs)
	# Always runs to catch subdomains missed by httpx tech detection
	wp_subdomain_ids.update(
		EndPoint.objects.filter(
			scan_history=self.scan,
			http_url__regex=r'wp-(content|includes|admin|login\.php)'
	...
	if not wp_subdomains.exists():
		logger.info('No WordPress targets detected, skipping WPScan')
		return []

	# Tag detected WordPress subdomains with "WordPress" technology
	wp_tech, _ = Technology.objects.get_or_create(name='WordPress')
	for sub in wp_subdomains:
		if not sub.technologies.filter(name='WordPress').exists():
			sub.technologies.add(wp_tech)
			sub.save()
			logger.info(f'Tagged {sub.name} with WordPress technology')

	# Get WPScan API key if available
	from dashboard.models import WPScanAPIKey
	api_key = WPScanAPIKey.objects.first()
	...
	for subdomain in wp_subdomains:
		...
		if not output or not output.strip():
			logger.info(f'No WPScan results for {subdomain.name}')
			continue

		try:
			scan_result = json.loads(output.strip())
		except json.JSONDecodeError as e:
			logger.error(f'Failed to parse WPScan output for {subdomain.name}: {e}')
			continue
		...
		version_info = scan_result.get('version', {})
		if version_info:
			for vuln in version_info.get('vulnerabilities', []):
				vuln_sources.append(('core', 'WordPress Core', vuln))

		# Main theme vulnerabilities
		main_theme = scan_result.get('main_theme', {})
		if main_theme:
			theme_name = main_theme.get('slug', 'unknown-theme')
			for vuln in main_theme.get('vulnerabilities', []):
				vuln_sources.append(('theme', theme_name, vuln))

		# Plugin vulnerabilities
		plugins = scan_result.get('plugins', {})
		for plugin_name, plugin_data in plugins.items():
			for vuln in plugin_data.get('vulnerabilities', []):
				vuln_sources.append(('plugin', plugin_name, vuln))

		for component_type, component_name, vuln in vuln_sources:
			vuln_data = parse_wpscan_result(vuln, component_name, component_type)

			vuln_obj, _ = save_vulnerability(
				target_domain=self.domain,
				http_url=target_url,
				scan_history=self.scan,
				subscan=self.subscan,
			...
			if vuln_obj:
				results.append({
					...
					'source': vuln_obj.source,
				})

	logger.info(f'WPScan completed, found {len(results)} vulnerabilities')
	return results


@app.task(name='http_crawl', queue='main_scan_queue', base=ReNgGinaNgTask, bind=True)
def http_crawl(
		self,
		urls=[],
		method=None,
...
def send_file_to_discord(file_path, title=None):
	...
	webhook = DiscordWebhook(
		...
		username=title or "ReNgGinaNg Discord Plugin"
	)
...
def _get_cvss_severity(cve_id):
	"""Lookup CVSS score for a CVE ID and return numeric severity.

	Args:
		cve_id (str): CVE ID (e.g. CVE-2024-1234)

	Returns:
		int or None: Severity (0-4) based on CVSS, or None if lookup fails.
	"""
	try:
		cve_info = CVESearch('https://cve.circl.lu').id(cve_id)
		if not cve_info:
			return None
		cvss = float(cve_info.get('cvss', -1))
		if cvss < 0:
			return None
		if cvss < 4:
			return 1  # low
...
def _get_title_severity(title):
	"""Infer severity from vulnerability title keywords.
	...
	critical_keywords = [
		...
		'object injection', 'sql injection', 'sqli',
		'privilege escalation', 'authentication bypass',
		'unauthenticated.*upload', 'arbitrary file write',
	]
	...
	# High (3)
	high_keywords = [
		'xss', 'cross-site scripting', 'file upload',
		'path traversal', 'directory traversal',
		'local file inclusion', 'lfi', 'ssrf',
		'server-side request forgery', 'file read',
		'arbitrary file', 'code injection', 'ssti',
		'template injection',
	]
	...
	medium_keywords = [
		...
		'open redirect', 'settings update',
		'unauthorized', 'broken access',
	]
	...
	# Low (1)
	low_keywords = [
		'disclosure', 'information leak', 'enumeration',
...
def parse_wpscan_result(vuln, component_name='', component_type=''):
	"""Parse WPScan vulnerability result.

	Severity is determined by:
	1. CVSS score from CVE lookup (if CVE exists)
	2. Keyword matching from vulnerability title (fallback)

	Args:
		vuln (dict): WPScan vulnerability data.
		component_name (str): Name of the WordPress component.
		component_type (str): Type of component (core, plugin, theme).

	Returns:
		dict: Vulnerability data for save_vulnerability().
	"""
	...
	cve_ids = []
	references = []
	for ref_type, ref_values in vuln.get('references', {}).items():
		if ref_type == 'cve':
			cve_ids = [f'CVE-{cve}' for cve in ref_values]
		elif ref_type == 'url':
			references = ref_values

	# Determine severity: CVE CVSS lookup first, then title keywords
	severity = None
	if cve_ids:
		severity = _get_cvss_severity(cve_ids[0])
		if severity is not None:
			logger.info(f'WPScan severity for "{title}" from CVSS: {severity}')

	if severity is None:
		severity = _get_title_severity(title)
		logger.info(f'WPScan severity for "{title}" from title keywords: {severity}')

	description = f'Component: {component_type} - {component_name}'
	if vuln.get('fixed_in'):
		description += f'<br>Fixed in version: {vuln["fixed_in"]}'

	return {
		'name': title,
		'type': vuln_type or 'WordPress',
		'severity': severity,
		'description': description,
		'source': WPSCAN,
		'cve_ids': cve_ids,
		'references': references,
		'tags': ['wordpress', component_type.lower()],
	}
...
def query_whois(target, force_reload_whois=False):
	...
	try:
		...
		whois_data = whois_data.get('data', {}) if whois_data else {}
...
def save_imported_subdomains(subdomains, ctx={}):
	...
	subdomains = list(set([
		...
		if validators.domain(subdomain) and get_domain_from_subdomain(domain.name) == get_domain_from_subdomain(subdomain)
	]))
	if not subdomains:
		return
